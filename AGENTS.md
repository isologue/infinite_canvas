...
- 每次写完代码，需要执行与改动相关的语法检查和测试；涉及可运行应用时，默认执行 `docker compose -f .\docker-compose.local.yml up -d --build` 完整构建并启动，再验证容器状态、关键接口和页面。若受环境或外部依赖阻塞，需要记录实际错误和已完成的验证。
- 不要改无关文件，不要顺手重构。
...
- 节点设置通过 Portal 挂载的浮层必须标记 `data-canvas-node-panel`，避免被画布的外部点击监听误关闭；修改关闭逻辑时同时验证浮层内部选项可操作和点击空白处可关闭。
