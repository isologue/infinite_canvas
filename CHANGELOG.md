...
## Unreleased
+ [优化] 页面 HTML 与 RSC 改为按请求生成并返回禁止缓存响应，避免 Docker 更新后反向代理继续复用旧页面并引用已删除的 Next.js 静态资源。

+ [修复] 修复 AI 调用日志的生图配置展示：展示实际请求尺寸并据此推导宽高比，不再重复展示分辨率；未发送给上游的质量明确显示为“未传递”。

+ [重构] 提示词来源改为管理员同步入库的正式资源，提示词正文、封面和参考图保存到 `prompt_resources`，普通用户预览不再回源外部图片地址。

+ [新增] 提示词资源支持 `active`、`upstream_missing` 和 `admin_deleted` 状态；上游缺失保留内容，管理员单独删除清理二进制并保留墓碑，来源删除物理清理该来源全部资源。

+ [新增] 管理后台资源管理增加“提示词资源”Tab，支持按来源、状态和关键词筛选、预览及批量删除；提示词资源不再执行 TTL、数量上限或自动清理。

+ [优化] 提示词资源管理显示全部提示词二进制资源总占用，列表增加标签展示并支持按标签搜索。

+ [新增] AI 调用日志新增生成总耗时，成功和失败记录均以秒展示，异步图片与视频包含任务创建、轮询等待和本地处理耗时。

+ [新增] 渠道配置增加图片返回格式，可选择 URL 或 Base64；默认选择 URL，图片请求默认发送 `response_format=url`。

+ [修复] AI 调用日志保留图片生成实际发送的请求参数和上游原始响应，异步任务同时记录创建与最终查询结果；修复画布丢失提示词违规等失败详情，并为长日志增加展开与收起查看。

+ [修复] 修复图片工作台异步创建和任务查询把渠道选择值中的渠道 ID 前缀一并作为模型名发送，导致上游提示模型无可用渠道的问题。

+ [优化] 图片工作台、视频工作台和画布生成节点统一支持异步任务持久化、每 5 秒自动轮询、手动刷新及页面刷新后恢复，并分别展示等待结果耗时和本地处理耗时。

+ [优化] MiniMax 视频设置补齐 8 种画幅比例和 480P/768P/1080P 尺寸映射，视频工作台可正确识别当前 MiniMax 模型，并支持自定义宽高原样传入 `size`。

+ [修复] 修复从我的资产插入参考图生成视频时 multipart 文件名缺少或使用错误扩展名，导致上游拒绝图片的问题。

+ [文档] 在 MiniMax 视频接口章节补充工作流 ID、参考素材限制以及比例与尺寸对应表。

+ [修复] 修复图片工作台生成记录全选后删除只删除一条的问题，改为一次性批量写回删除结果。

+ [文档] 更新外部统一 API 文档，将原视频分类调整为「OPENAI 视频」，并新增 MiniMax 视频创建、任务查询和内容下载接口说明。

+ [修复] 修复视频工作台生成记录全选后删除只删除一条的问题，改为一次性批量写回删除结果。

+ [优化] 视频任务创建成功后按上游进度显示轮询状态，每 5 秒自动查询并支持手动刷新；取消固定轮询次数限制，页面刷新或重新打开生成记录后可继续恢复未完成任务。

+ [新增] 视频渠道新增 MiniMax H3 协议，支持工作流、图片、视频和音频参考素材上传，以及异步任务查询和视频下载。

+ [新增] 画布底部工具条增加批量下载，支持单击多选或拖动框选节点，并在打包前自定义压缩包名称。
+ [优化] 双击画布节点自动放大并居中，节点功能菜单条跟随画布缩放同步缩放。
+ [优化] 上传或替换参考图片时，图片节点会按当前画布缩放自动适配可视区域，避免超大图片压缩整体画布视野。
+ [修复] 修复中键拖动画布在松开后偶发保持抓手状态的问题。
+ [修复] 已连接参考图的图片结果节点重复生成时继续覆盖自身，不再额外创建图片节点或把旧结果误作唯一参考图。
+ [优化] 等比图片节点拖拽缩放时仅限制最长边最小尺寸，可继续按原始比例缩小。
+ [修复] 替换已手动缩放的图片时保持节点视觉尺寸与中心位置，仅按新图片比例重算宽高。
+ [优化] 批量下载压缩包仅包含选中的文字、图片、视频、音频等实际资源，不再附带 nodes.json 或其他节点 JSON。

+ [优化] 画布侧边栏的组资产卡片显示组名称，便于区分和复用多个组。

+ [优化] 成组确认弹窗要求填写唯一组名，并阻止当前画布或我的资产中出现重复组名。

+ [新增] 画布组可保存为我的资产，并可在新画布中复用组内节点、连线、提示词和媒体引用；组资产关联资源会标记为已保存，避免被定时清理。

+ [重构] 画布组改为框选确认创建，组节点支持顺序批量运行和解散保留成员节点。

+ [新增] 选择工具支持在画布空白处左键拖动框选多个节点，按 Shift 可追加选区。
+ [修复] 复制画布节点时保留其与未选中节点的连线，粘贴后仅替换复制节点端点。
+ [优化] 服务端视频转存允许域名支持 `*.example.com` 形式的通配子域名匹配，适配上游动态视频 CDN 域名。
+ [修复] 兼容视频上游以 `code: success` / `ok` 表示任务成功及嵌套视频地址的响应，避免已生成视频被错误记录为失败。
+ [优化] AI 调用日志请求参数新增参考图、参考视频和参考音频的传入状态与数量，不记录资产原始内容。
+ [调整] 提示词来源配置仅向超级管理员显示，普通用户仍可使用提示词库，但无法通过配置页或配置文件查看、导入和导出来源配置。
+ [修复] 修复 Next/Turbopack 下节点插件动态安装失败，以及重新进入画布后已存储图片无法恢复显示的问题。
+ [修复] 修复上游 Vite 前端更新合并到 Next 部署后配置入口消失、画布创建后无法进入，以及移动端导航、版本信息和本地插件清单不兼容的问题。
+ [修复] 自定义 AI 渠道改用同源代理请求，解决上游未开放 CORS 时无法拉取模型和生成的问题。
+ [新增] 渠道兼容Gemini格式。
