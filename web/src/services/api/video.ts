...
import { buildAiProxyUrl, buildApiUrl, modelOptionName, resolveModelRequestConfig, type AiConfig } from "@/stores/use-config-store";
...
export async function requestVideoGeneration(
    ...
    try {
        ...
        if (task) await settleVideoTaskCredits(task, "failed", { errorMessage: error instanceof Error ? error.message : String(error) });
        throw error;
    }
}
