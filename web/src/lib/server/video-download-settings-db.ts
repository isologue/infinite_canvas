...
export function normalizeVideoDownloadSettings(input: unknown, strict = false): VideoDownloadSettings {
    ...
    if (strict && normalizedHosts.some((host) => !host)) throw new Error("Allowed video hosts must be complete domain names or wildcard subdomains");
...
function normalizeHost(value: unknown) {
    ...
    if (!host || input.length > 253 || !/^[a-z0-9](?:[a-z0-9.-]*[a-z0-9])?$/.test(host) || !host.includes(".") || /^\d+(?:\.\d+){3}$/.test(host)) return "";
