...
export function ChannelEditorDrawer({ open, channel, canManageUrl = true, lockedBaseUrlOptions = [], onSave, onClose }: { open: boolean; channel: ModelChannel | null; canManageUrl?: boolean; lockedBaseUrlOptions?: string[]; onSave: (channel: ModelChannel) => void; onClose: () => void }) {
    ...
    const save = () => {
        onSave({ ...draft, name: draft.name.trim() || "未命名渠道", baseUrl: canManageUrl ? draft.baseUrl : lockedBaseUrlOptions[0] || draft.baseUrl, models: normalizeChannelModels(draft.models) });
        onClose();
    };
        ...
        >
            <div className="grid gap-4 md:grid-cols-2">
                ...
                <label className="block md:col-span-2">
                    ...
                    {canManageUrl ? <Input value={draft.baseUrl} onChange={(event) => patch({ baseUrl: event.target.value })} placeholder="https://api.example.com" /> : <Select className="w-full" value={draft.baseUrl || lockedBaseUrlOptions[0]} options={lockedBaseUrlOptions.map((value) => ({ label: value, value }))} onChange={(baseUrl) => patch({ baseUrl })} />}
                </label>
                <label className="block md:col-span-2">
