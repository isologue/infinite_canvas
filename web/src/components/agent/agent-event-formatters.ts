...
export function agentMessageToChatMessage(item: AgentChatItem, endpoint = "", token = "") {
...
export function agentAttachmentToChatAttachment(item: AgentMessageAttachment | AgentAttachment, endpoint = "", token = ""): AgentChatAttachment {
    const source = item.dataUrl || item.url;
    return { id: item.id, name: item.name, url: endpoint && token ? resolveAgentMessageAssetUrl(endpoint, token, source) : source };
}
...
export function mergeHistoryAttachments(messages: AgentChatItem[], currentMessages: AgentChatItem[]) {
    return messages.map((item) => {
        const current = currentMessages.find(({ threadId, text, historyText }) => threadId === item.threadId && (text === item.text || historyText === item.text));
        return current ? { ...item, text: current.text, historyText: current.historyText, attachments: current.attachments } : item;
    });
}
